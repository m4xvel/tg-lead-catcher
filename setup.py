...
Отдельная точка входа: `python setup.py`. Не импортируется рантаймом
...
`tests/test_setup.py` с фейковым Telethon-клиентом, без реального ввода и
...
async def login_userbot(
    ...
    if not signed_in and not needs_password:
        raise LoginAborted(
            "Код подтверждения введён неверно несколько раз подряд — запустите setup.py заново"
        )
    ...
    if needs_password:
        ...
        if not password_ok:
            raise LoginAborted(
                "2FA-пароль введён неверно несколько раз подряд — запустите setup.py заново"
            )
