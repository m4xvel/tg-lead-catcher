...
Тесты, полный набор (из корня): `python -m pytest` — 193 passed на момент последней сборки
...
- Хендлеры панели, ждущие свободный текст через FSM (`waiting_value`/`waiting_bulk`/`waiting_check_text`/`waiting_manual`/`waiting_forward`/`waiting_search`), принимают ЛЮБОЙ текст, включая команды — `CommandStart()`/`Command("export")` без фильтра состояния перехватывают апдейт только если зарегистрированы раньше в том же роутере; там, где FSM-хендлер зарегистрирован первым, команда долетает как «введённое значение» вместо того, чтобы сработать как команда, а там, где раньше зарегистрирован сам обработчик команды (`cmd_start` в `panel/sources.py`, `cmd_export` в `panel/export.py`) — команда реально выполняется, но FSM state не чистится и остаётся «завис». Оба случая теперь чинятся симметрично: все шесть FSM-хендлеров тихо отменяют ввод на любой текст, начинающийся с `/` (`state.clear()` + возврат к виду кнопки «Отмена»), а `cmd_start`/`cmd_export` сами зовут `state.clear()` в начале. Любая новая команда (`Command(...)`) или новый FSM-ввод со свободным текстом — повторяй этот паттерн, не полагайся на порядок роутеров в `panel/__init__.py`.
