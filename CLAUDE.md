...
- Хендлеры панели, ждущие свободный текст через FSM (`waiting_value`/`waiting_bulk`/`waiting_check_text`/`waiting_manual`/`waiting_forward`/`waiting_search`), принимают ЛЮБОЙ текст, включая команды — `CommandStart()`/`Command("export")` без фильтра состояния перехватывают апдейт только если зарегистрированы раньше в том же роутере; там, где FSM-хендлер зарегистрирован первым (как было с `/start` в `panel/sources.py`), команда долетает как «введённое значение» вместо того, чтобы сработать как команда. Все шесть таких хендлеров теперь тихо отменяют ввод на любой текст, начинающийся с `/` (`state.clear()` + возврат к виду кнопки «Отмена» того же потока) — при добавлении нового FSM-ввода со свободным текстом повторяй этот паттерн, не полагайся на порядок роутеров.
